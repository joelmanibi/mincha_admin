    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        ...
        <Tabs defaultValue="info" className="w-full">
          <TabsList className={`grid w-full ${showValidationTab ? 'grid-cols-4' : 'grid-cols-3'}`}>
            <TabsTrigger value="info">Informations</TabsTrigger>
            <TabsTrigger value="documents">Documents</TabsTrigger>
            <TabsTrigger value="account">Compte professionnel</TabsTrigger>
            {showValidationTab && (
              <TabsTrigger value="validation">Validation du compte</TabsTrigger>
            )}
          </TabsList>
          ...
          <TabsContent value="account" className="space-y-6">
            {user?.account ? (
                ...
                <div className="flex items-start space-x-4">
                  {user.account.accountLogo && (
                    <div className="h-24 w-24 rounded-lg bg-gray-100">
                      <img
                        src={`http://dev-mani.tech:8000/static/account/logo_file/${user.account.accountLogo}`}
                        alt="Logo du compte"
                ...
                <div className="grid gap-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    {user.account.accountDoc && (
                      <div>
                        ...
                        <a
                          href={`http://dev-mani.tech:8000/static/account/doccument_file/${user.account.accountDoc}`}
                          target="_blank"
                  ...
                  <div className="grid grid-cols-2 gap-4 border-t pt-4">
                    <div>
                      ...
                      <p className="mt-1">
                        {format(new Date(user.account.createdAt), "PPP", { locale: fr })}
                      </p>
                    </div>
                    <div>
                      <h4 className="text-sm font-medium text-gray-500">Dernière mise à jour</h4>
                      <p className="mt-1">
                        {format(new Date(user.account.updatedAt), "PPP", { locale: fr })}
                      </p>
                    </div>
                  </div>
                </div>
              ...
              <div className="text-center py-8 text-muted-foreground">
                Aucun compte professionnel associé
              </div>
            )}
          </TabsContent>
